...
internal object AgentRuntimeWire {
    ...
    const val MSG_ENTRY_TOOL_REQUEST = 13

    /** client -> 单次回调 Messenger：返回入口工具结果。 */
    const val MSG_ENTRY_TOOL_RESULT = 14

    /** service -> XiaoAi client：请求在超级小爱进程内执行一项白名单 ToolsBridge 工具。 */
    const val MSG_XIAOMI_TOOLS_BRIDGE_CALL = 15

    /** XiaoAi client -> service：返回一项 ToolsBridge 工具的终态结果。 */
    const val MSG_XIAOMI_TOOLS_BRIDGE_RESULT = 16

    /** service -> injected host：执行一个仅宿主 UID 可用的工具调用。 */
    const val MSG_HOST_CALL = 17

    /** injected host -> service：宿主工具的流式事件。 */
    const val MSG_HOST_EVENT = 18

    /** injected host -> service：宿主工具的唯一终态。 */
    const val MSG_HOST_RESULT = 19
