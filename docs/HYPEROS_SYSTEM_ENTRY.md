...
- 拦截：返回 `boolean` 的 `triggerFunction(String, String, Bundle, boolean)` 与末尾增加 `String` 的五参数重载。`HyperOsPowerHooks` 统一委托分支已有的 `PowerHooks` 实现，避免同一快捷动作重复安装两套拦截；运行时校验 `mContext`。
- 触发条件：第一个参数为 `launch_voice_assistant`，第二个参数为 `long_press_power_key` 或 `imperceptible_press_power_key`。后者是部分系统版本的短时长电源键助手路径。
- 执行：读取现有电源键目标并结合当前默认数字助理解析。系统默认助手直接放行；Gemini / Eta 优先通过 `AssistantManager.showAssistantSession` 请求会话，并保留分支已有的 Activity 兜底和触发去重。非厂商目标未安装或启动失败时阻止重新拉起超级小爱。

该入口在系统识别完按键动作后接管，不重写按键计时或触摸事件，也不修改系统电源键设置。关机菜单、SOS、双击相机、智能家居和非电源键助手动作不符合匹配条件，保持系统逻辑。当前用户需已启用电源键助手快捷方式；沿用 Eta 的“自动设置默认助理”开关与已有恢复机制。对于绕过快捷分发 Hook 的优化调用，继续拦截 `launchVoiceAssistant(String, Bundle)` 最终出口，仅在用户选择 Eta / Gemini 时改走所选目标。

四参数重载和桌面手势支持来自上游，五参数入口与最终出口沿用分支已有适配；不能据此断言所有 HyperOS 版本的调用链、字段和按住松手时序相同。未知类、字段和签名通过安装报告诊断，不按模糊方法名拦截。本次合并尚未进行新的真机验证。
