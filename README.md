...
Eta 在同一个 APK 中支持普通设备与 Root 设备。普通用户可以使用聊天、无障碍 GUI、浏览器、Android Shell、Linux、Kimi Web、Skills 和 MCP；获得 Root 或启用 LSPosed 后，还能使用对应的系统修改、私有数据读取与厂商助手入口。所有能力都交给你自己接入的模型（ChatGPT、DeepSeek、Kimi 等，自带 API Key——BYOK），并按设备授权决定实际可用范围：

- **系统 API 直达**：闹钟、媒体、音量、Wi‑Fi 等系统能力，模型可直接调用
- **个人上下文**：授权后的通知、应用使用情况和位置可供普通用户使用；相册、日历、短信、录音、健康摘要、ColorOS 系统记忆与聊天图片等专用读取依赖 Root 或对应系统能力
...
工具页默认展示“当前设备”，普通 Android 授权尚未开启的能力仍可查看。切换“全部能力”可以了解 Root 与厂商能力，查看介绍不会申请权限，也不会向 Agent 开放额外工具。设置页的“系统增强”分别显示 Root 与 LSPosed 状态。
...
> [!NOTE]
> 支持 **Android 14+**，基础功能不依赖 Root 或 LSPosed。Root 系统工具需要成功授权；厂商 Hook 需要支持 libxposed API 102 的 LSPosed 与对应 ROM。App 本体不限 OPPO 或小米设备；ColorOS（小布助手）与 HyperOS（小爱同学）只是当前系统助手入口的适配范围。
...
- **共享文件夹**：把有权访问的 Android 目录映射到 Linux 的 `/workspace/mounts/`；普通用户可按需授权“所有文件访问”，也可始终使用私有工作区与文件导入导出
- **文件浏览**：直接在 App 里浏览和预览 Linux 环境内的文件

普通 Linux 使用 PRoot，以 App UID 运行；Root Linux 继续使用 chroot。两种后端的环境独立安装，已有环境、会话和数据不会因 Root 状态变化而自动迁移。PRoot 内显示的 root 仅为 Linux 用户态模拟身份，不提供 Android Root 权限。

### 内置 Kimi Code

- 安装所选 Linux 环境及 Kimi Code 后，在首页启动 Kimi Web
- 同一发行版与后端复用正在运行的实例；切换浏览器或退出页面后仍可返回打开，并可主动停止
- 普通终端和 Kimi 由执行前台服务持有，通知权限按需申请，拒绝不会直接阻断启动；系统强停、重启或回收进程后不自动重放命令
...
在设置页的“系统助手接管”中可选择 ColorOS 或 HyperOS 长按电源键的目标：

| 目标     | 长按后的行为               | 默认助理自动设置        |
| -------- | -------------------------- | ----------------------- |
| 厂商助手 | 保留小布或超级小爱原始行为 | 不修改系统默认助理      |
| Gemini   | 沿用 Google 的系统助手链路 | 开关开启时切换为 Gemini |
| Eta      | 打开 Eta 全屏键盘助理浮窗  | 开关开启时切换为 Eta    |

新安装默认保持厂商助手；旧版已经开启“长按电源键唤起 Gemini”的用户会继续使用 Gemini。“自动设置默认助理”是独立开关，只对 Gemini 和 Eta 生效。ColorOS 当前目标无法启动时，本次长按会立即回退到小布；HyperOS 会接管小米快捷键分发器及其最终写死的超级小爱启动出口，改走所选目标，非厂商目标启动失败时不会重新拉起小爱。默认助理校正在 `system_server` 开机阶段完成，不依赖 `service.d`。
...
- **Fast 可选加速**：聊天输入框的推理等级菜单新增独立 Fast 开关，默认关闭，可与任意推理等级组合；所有模型（包括无推理档位的模型）均可尝试，切换模型后保留开关状态。服务端明确拒绝加速参数时自动回退一次普通请求；实际加速取决于提供商与账户，可能额外计费。
...
2. 按需授予无障碍、应用列表读取、位置、通知使用权、使用情况访问和后台运行权限；GUI 反馈使用无障碍浮层，Eta 原生助理面板另按需申请悬浮窗权限；后台位置任务需授予“始终允许”
3. 按需开启设备直达、敏感信息读取、敏感设备操作和终端/文件工具；可在“上下文与扩展”中添加远程 MCP 服务器并逐项启用需要的工具；无 Root 时终端仅提供 `user`，Linux 使用 PRoot；Root 用户保留 `root` 与 chroot。选择 Alpine 或 Debian，安装基础环境后再安装所需工具
...
- 本地工具具有统一权限合同。每轮移除不可用工具并收窄混合工具的参数，再按同一份 Schema 校验调用；执行前重新检查 Root，旧调用不能绕过。用户保存的开关不会因失联或撤权被覆盖；远程 MCP 继续使用独立授权
...
- 记忆读写同样只供当前回合使用，持久会话只保留脱敏操作摘要；聊天文件引用按 App UID 或已授权 URI 读取；无法直接访问的选择器文件先导入私有工作区，再引用导入后的路径。Root 用户保留原有特权路径
- 前台 GUI 操作显示运行浮层与手势反馈，用户可随时停止或接管

## 边界与限制

- **普通设备边界**：本次没有新增联系人、短信或日历的普通权限版读取；未满足条件的工具不会提供给模型。PRoot 不是 Android 权限绕过机制
- **后台运行**：前台服务仍受 Android 与厂商进程管理约束；后台启动被拒绝时需回到 Eta 重试，应用强停或设备重启后手动重新启动任务
- **验证范围**：代码、单元测试与 APK 打包检查不能代替真机验收；Android 14+ 的授权弹窗、PRoot/PTY 运行、浏览器切换与厂商后台策略需按[验收清单](docs/ROOTLESS_SUPPORT.md)验证
...
本项目走第三方开发者路线：不代表手机厂商，不需要维护预装合作。用户可以先使用普通 Android 授权下的功能，也可以自行选择 Root 与 Xposed 扩展系统能力，把手机入口接给自己选择的 Agent。风险边界由用户决定，工具必须透明可见，敏感操作必须能随时停止和接管。
